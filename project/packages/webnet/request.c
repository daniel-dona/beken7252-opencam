        ...
        *ch ++ = '\0';
        ...
        else if (str_begin_with(request_buffer, "Accept-Encoding:"))
        {
            ...
            if( (gzip != RT_NULL))
            {
                ...
                //WEBNET_PRINTF("client support gzip\n");
            }
        }
    ...
    for (;;)
    {
        if (str_begin_with(request_buffer, "\r\n"))
        {
            ...
            if (request->method == WEBNET_POST && request->content_length > 0) /* POST method */
            {
                ...
                {
                    rt_kprintf("content lengt too long: %d, discard it.\n", request->content_length);
                ...
                if (read_ptr == RT_NULL)
                {
                    rt_kprintf("WebRequest: out of memory!\n");
                    request->query = RT_NULL;
                    return ;
                }

                /* get the length of already read bytes */
                read_length = (rt_uint16_t)(length - ((rt_uint32_t)request->query - (rt_uint32_t)buffer));
                if (read_length > 0)
                {
